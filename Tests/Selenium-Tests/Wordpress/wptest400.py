...
class WPTest400(WPTest):
    def __init__(self):
        # UPDATE HERE (1/5)
        self.main_page = 'http://localhost:8085/WordPress-4.0/'
        #super().__init__(self.main_page)
        super(WPTest400, self).__init__(self.main_page)
    ...
    def add_cookies(self):
        self.driver.get(self.main_page)
        # UPDATE HERE (2/5)
        self.driver.add_cookie({'name': 'test_group', 'value': 'wp400_tutorials'})
        # UPDATE HERE (3/5)
        self.driver.add_cookie({'name': 'test_name', 'value': 'wp_login_wp400_tutorials'})
        # UPDATE HERE (4/5)
        self.driver.add_cookie({'name': 'software_id', 'value': '4'})
        # UPDATE HERE (5/5)
        self.driver.add_cookie({'name': 'software_version_id', 'value': '16'})
    ...
    def set_feature_image_by_uploading(self, image_path):
        ...
        self.click_element('//*[@id="__attachments-view-41"]/li')

        #self.upload_file_input('//*[@id="__wp-uploader-id-2"]/div[@class="moxie-shim moxie-shim-html5"]/input',
        #                       image_path)

        if self.wait_for_text_in_page("Edit Image"):
            time.sleep(1)
            self.click_element(
                '//*[@id="__wp-uploader-id-0"]/div[5]/div/div[2]/a')  # Can be improved by class name
    ...
    def upload_theme(self, theme_path):
        ...
        if not self.wait_for_text_in_page('Upload Theme'):
            ...
            return
        self.click_element('//a[text()="Upload Theme"]')
        #self.click_element('//*[@id="wpbody-content"]/div[4]/h2/a[1]')
    ...
    def quick_edit_category(self, id, new_name, new_slug, is_tag=False):
        ...
        self.click_element('//a[contains(text(), "Update ")]')
    ...
    def approve_comment(self, comment_id):
        ...
        ele_xpath = '//*[@id="comment-%d"]//a[contains(text(), "Approve")]' % comment_id
        if self.driver.find_element_by_xpath(ele_xpath).is_displayed():
            self.click_element(ele_xpath)
        else:
    ...
    def unapprove_comment(self, comment_id):
        ...
        ele_xpath = '//*[@id="comment-%d"]//a[contains(text(), "Unapprove")]' % comment_id
        hover = ActionChains(self.driver).move_to_element(
    ...
    def reply_comment(self, comment_id, reply_content):
        ...
        ele_xpath = '//*[@id="comment-%d"]//a[contains(text(), "Reply")]' % comment_id
        if self.driver.find_element_by_xpath(ele_xpath).is_displayed():
            self.click_element(ele_xpath)
        else:
            self.success = False
    ...
    def quick_edit_comment(self, comment_id, new_author=None, new_email=None, new_website=None, new_content=None):
        ...
        ele_xpath = '//*[@id="comment-%d"]//a[contains(text(), "Quick Edit")]' % comment_id
        if self.driver.find_element_by_xpath(ele_xpath).is_displayed():
            self.click_element(ele_xpath)
        else:
            self.success = False
    ...
    def spam_comment(self, comment_id):
        ...
        ele_xpath = '//*[@id="comment-%d"]//a[contains(text(), "Spam")]' % comment_id
        if self.driver.find_element_by_xpath(ele_xpath).is_displayed():
            self.click_element(ele_xpath)
        else:
            self.success = False
    ...
    def move_comment_to_trash(self, comment_id):
        ...
        ele_xpath = '//*[@id="comment-%d"]//a[contains(text(), "Trash")]' % comment_id
        if self.driver.find_element_by_xpath(ele_xpath).is_displayed():
            self.click_element(ele_xpath)
        else:
            self.success = False
    ...
    def edit_comment(self, comment_id, new_author=None, new_email=None, new_url=None, new_content=None, new_status=None,
        ...
        # Check datetime in format like Nov 15, 2018 @ 20:19
        publishing_datetime = '%s %s, %s @ %s:%s' % (trans_mon_str[int(month)], day, year, hour, minute)
        if publishing_datetime == self.driver.find_element_by_xpath('//*[@id="timestamp"]/b').text:
            print('[+] Changing publishing datetime successfully')
        else:
            self.success = False
    ...
    def delete_comment(self, comment_id):
        ...
        self.click_element(
            '//*[@id="comment-%d"]//a[contains(text(), "Delete Permanently")]' % comment_id)
    ...
    def unspam_comment(self, comment_id):
        ...
        self.click_element('//*[@id="comment-%d"]//a[contains(text(), "Not Spam")]' % comment_id)
    ...
    def edit_user_color_scheme(self, scheme_idx=0, user_id=None):
        ...
        if user_id is None:
            ...
            self.click_element('//*[@id="user-%d"]//a[contains(text(), "Edit")]'
                               % user_id)
    ...
    def user_test(self):
        ...
        user_id = self.add_user(username='new_user1' + self.get_random_text(3, 10), email='new_user1%s@example.com' % self.get_random_text(3, 10), password='123456')
    ...
    def delete_media(self, media_id):
        ...
        self.click_element('//*[@id="post-%d"]//a[contains(text(), "Delete Permanently")]' % media_id)
...
if __name__ == '__main__':
    ...
    test.driver.add_cookie({'name': 'test_name', 'value': 'wp_post_wp400_tutorials'})
    test.new_post_tests() if test.success else None
    test.driver.add_cookie({'name': 'test_name', 'value': 'wp_theme_wp400_tutorials'})
    test.theme_tests() if test.success else None
    test.driver.add_cookie({'name': 'test_name', 'value': 'wp_setting_wp400_tutorials'})
    test.setting_tests() if test.success else None
    test.driver.add_cookie({'name': 'test_name', 'value': 'wp_category_wp400_tutorials'})
    test.category_tag_tests() if test.success else None
    test.driver.add_cookie({'name': 'test_name', 'value': 'wp_comment_wp400_tutorials'})
    test.comment_tests() if test.success else None
    test.driver.add_cookie({'name': 'test_name', 'value': 'wp_export_wp400_tutorials'})
    test.export('all') if test.success else None
    test.driver.add_cookie({'name': 'test_name', 'value': 'wp_user_wp400_tutorials'})
    test.user_test() if test.success else None
    test.driver.add_cookie({'name': 'test_name', 'value': 'wp_media_wp400_tutorials'})
