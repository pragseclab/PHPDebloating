...
class WPTest423(WPTest):
    def __init__(self):
        # UPDATE HERE (1/5)
        self.main_page = 'http://localhost:8085/WordPress-4.2.3/'
        #super().__init__(self.main_page)
        super(WPTest423, self).__init__(self.main_page)
    ...
    def add_cookies(self):
        self.driver.get(self.main_page)
        # UPDATE HERE (2/5)
        self.driver.add_cookie({'name': 'test_group', 'value': 'wp423_tutorials'})
        # UPDATE HERE (3/5)
        self.driver.add_cookie({'name': 'test_name', 'value': 'wp_login_wp423_tutorials'})
        # UPDATE HERE (4/5)
        self.driver.add_cookie({'name': 'software_id', 'value': '4'})
        # UPDATE HERE (5/5)
        self.driver.add_cookie({'name': 'software_version_id', 'value': '17'})
    ...
    def quick_edit_comment(self, comment_id, new_author=None, new_email=None, new_website=None, new_content=None):
        ...
        ele_xpath = '//*[@id="comment-%d"]//a[contains(text(), "Quick")]' % comment_id
        if self.driver.find_element_by_xpath(ele_xpath).is_displayed():
            self.click_element(ele_xpath)
        else:
            self.success = False
